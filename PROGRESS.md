...
- Plan: raise sampling fidelity (sub-pixel luminance, richer finder refs, 7x7 votes), add format-info recovery for mask/ECL, hybrid direct-grid vs generator-with-recovered-params, thread full rich info to Batch Recast for exact recasts, consistent quiet zone.
- Implemented: upgraded getModulesFromImageData with bilinear sub-pixel lum sampling + expanded finder refs for threshold + 7x7 votes; added recoverMaskAndEclFromGrid using format info table for exact mask/ECL recovery from photo; createQrSvg now honours recovered maskPattern+ecl when forcing version (no modulesGrid); decode now recovers always, validates modulesGrid (q=4) and drops only grid on fail; threaded modulesGrid+maskPattern+errorCorrectionLevel through HomeView, ResultView, Batch store/thumbnail/exporters so batch gets faithful recasts too; raised image res to 3000px and video decode to 960px; quiet zone 4 uniform; lint+build clean; pure recovery verified on generated grids.
- Verified: npm run lint + npm run build clean after prettier; local preview serves; QR-original.jpg path now uses improved high-res sampling + recovery/hybrid. Batch and single consistent. No extra deps.
- Ran fresh lint + build post-commit prep; committed "feat: improve photographed QR fidelity with higher-accuracy sampling and mask recovery"; pushed 3f11eee to main. GitHub Actions will deploy.
- Updated PROGRESS and RECENT_CHANGES.
